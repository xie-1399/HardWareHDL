...
/*
can add the Pipeline regs throw addReg(hardware)
*/
...
class PipelineRegs(stage: Stage) extends Component {
  ...
  def addReg[T<:Data](data:PipelineData[T]):(T,T) = rework{
    ...
    parent.rework{ shift:= False} //Todo
    val reg = RegNextWhen(input,shift||regShift).init(input.getZero) // Todo
    output := reg
  ...
  def setReg[T<:Data](data:PipelineData[T],value:T):Unit = {
    val regInfo = regs(data.asInstanceOf[PipelineData[Data]]).asInstanceOf[RegInfo[T]]
    regInfo.input := value
    regInfo.shift := True
  }
